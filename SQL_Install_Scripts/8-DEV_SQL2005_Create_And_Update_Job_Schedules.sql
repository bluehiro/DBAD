...
--20130716DO Major organization overhaul of the entire script
	...
	--20141226DO Took the Prod version and created this Dev version.
		--Major change is scheduling System and User database backups to run once per week, insead of daily
		 --Therefore modified these two sections
...
SELECT @IndOpt_StartTime = REPLACE(convert(varchar,@StartTime,108), ':', '') 
SELECT @StartTime = DATEADD(mi, 90, @StartTime)
SELECT @ICUDB_StartTime = REPLACE(convert(varchar,@StartTime,108), ':', '')
SELECT @StartTime = DATEADD(mi, 60, @StartTime)
SELECT @ICSDB_StartTime = REPLACE(convert(varchar,@StartTime,108), ':', '')
SELECT @StartTime = DATEADD(mi, 60, @StartTime)
SELECT @SPBH_StartTime = REPLACE(convert(varchar,@StartTime,108), ':', '')
SELECT @StartTime = DATEADD(mi, 60, @StartTime)
SELECT @SPJH_StartTime = REPLACE(convert(varchar,@StartTime,108), ':', '')
...
--- Set Job schedules at correct intervals---
SELECT @OHSU_FreeSpace_StartTime = REPLACE(convert(varchar,@OHSU_StartTime,108), ':', '') 
SELECT @OHSU_StartTime = DATEADD(mi, 0, @OHSU_StartTime) --Change 0 to the desired time difference between the two schedules
SELECT @OHSU_DbSize_StartTime = REPLACE(convert(varchar,@OHSU_StartTime,108), ':', '')
...
--- Create Weekly Schedule for DatabaseBackup - SYSTEM_DATABASES - FULL ---
	DECLARE 
		@StartTime_SDFD datetime
	...
	SELECT @LowestHour_SDFD = 08 ---- The lowest random number
	SELECT @HighestHour_SDFD = 20 ---- The highest random number
	...
	-- Set Day of Week --
	SELECT @DayOfWeek_SDFD = 
		Case	WHEN @MinInterval_SDFD < 30 then 1
					WHEN @MinInterval_SDFD >=30 then 64
		End
	...
	-- Set Job schedules at correct intervals --
	SELECT @Schedule_StartTime_SDFD = REPLACE(convert(varchar,@StartTime_SDFD,108), ':', '') 

	--Select @DayOfWeek_SDFD as DayOfWeek, @HourInterval_SDFD as HourInterval, @MinInterval_SDFD as MinInterval, @Schedule_StartTime_SDFD as Schedule_StartTime
	...
	EXEC @ReturnCode_SDFD = msdb.dbo.sp_add_schedule --@job_id=@jobId,
		@Schedule_name=N'DatabaseBackup - SYSTEM_DATABASES - FULL - Weekly', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval= @DayOfWeek_SDFD, 
		@freq_subday_type=1, 
		@freq_subday_interval=0, 
		@freq_relative_interval=0, 
		@freq_recurrence_factor=1, 
...
--- /Create Weekly Schedule for DatabaseBackup - USER_DATABASES - FULL ---
	DECLARE 
		@StartTime_UDFD datetime
		...
		,@DayOfWeek_UDFD INT
		,@ReturnCode_UDFD INT
		,@JobStartDate_UDFD varchar(10)
	SELECT @ReturnCode_UDFD = 0

	-- Set Value Limits --
	SELECT @StartTime_UDFD = '00:00:00'
	SELECT @LowestHour_UDFD = 08 ---- The lowest random number
	SELECT @HighestHour_UDFD = 20 ---- The highest random number
	SELECT @LowestMin_UDFD = 0 ---- The lowest random number
	SELECT @HighestMin_UDFD = 59 ---- The highest random number
	SELECT @JobStartDate_UDFD = convert(varchar,getdate(),112) -- Set the job start date
	...
	-- Set Day of Week --
	SELECT @DayOfWeek_UDFD = 
		Case	WHEN @MinInterval_UDFD < 30 then 1
					WHEN @MinInterval_UDFD >=30 then 64
		End
	...
	-- Set Job schedules at correct intervals --
	SELECT @Schedule_StartTime_UDFD = REPLACE(convert(varchar,@StartTime_UDFD,108), ':', '') 

	--Select @DayOfWeek_UDFD as DayOfWeek, @HourInterval_UDFD as HourInterval, @MinInterval_UDFD as MinInterval, @Schedule_StartTime_UDFD as Schedule_StartTime
	...
	EXEC @ReturnCode_UDFD = msdb.dbo.sp_add_schedule --@job_id=@jobId,
		@schedule_name=N'DatabaseBackup - USER_DATABASES - FULL - Weekly', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval= @DayOfWeek_UDFD, 
		@freq_subday_type=1, 
		@freq_subday_interval=0, 
		@freq_relative_interval=0, 
		@freq_recurrence_factor=1, 
		@active_start_date=@JobStartDate_UDFD,
		@active_end_date=99991231, 
...
--- Create Weekly Schedule for Output File Cleanup ---
	...
	SELECT @Schedule_StartTime_OFC = REPLACE(convert(varchar,@StartTime_OFC,108), ':', '') 
