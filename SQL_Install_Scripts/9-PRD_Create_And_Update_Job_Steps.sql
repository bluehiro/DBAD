...
DECLARE @JobID nvarchar(200)
		...
		Select @JobID = job_id FROM msdb.dbo.sysjobs WHERE [name] = 'DatabaseBackup - SYSTEM_DATABASES - FULL'
			EXEC msdb.dbo.sp_update_jobstep @job_id=@JobID, @step_id=1 , 
					@command=N'IF 01 == %DATE:~7,2% (sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d ohsu_sqltab -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''SYSTEM_DATABASES'', @Directory = N''\\msbdd1b\msb_sql\Prod\Monthly'', @BackupType = ''FULL'', @Verify = ''N'', @CopyOnly = ''N'', @Compress = ''N'', @CleanupTime = 9672, @CheckSum = ''Y'', @LogToTable = ''Y''" -b) ELSE IF Mon == %DATE:~0,3% (sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d ohsu_sqltab -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''SYSTEM_DATABASES'', @Directory = N''\\msbdd1b\msb_sql\Prod\Weekly'', @BackupType = ''FULL'', @Verify = ''Y'', @CopyOnly = ''N'', @Compress = ''N'', @CleanupTime = 2184, @CheckSum = ''Y'', @LogToTable = ''Y''" -b) ELSE (sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d ohsu_sqltab -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''SYSTEM_DATABASES'', @Directory = N''\\msbdd1b\msb_sql\Prod\Daily'', @BackupType = ''FULL'', @Verify = ''Y'', @CopyOnly = ''N'', @Compress = ''N'', @CleanupTime = 504, @CheckSum = ''Y'', @LogToTable = ''Y''" -b)'
					,@retry_attempts=2
					,@retry_interval=2
					--IF 01 == %date:~7,2% (sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d ohsu_sqltab -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = 'SYSTEM_DATABASES', @Directory = N'\\msbdd1b\msb_sql\Prod\Monthly', @BackupType = 'FULL', @Verify = 'N', @CopyOnly = 'Y', @Compress = 'N', @CleanupTime = 9672, @CheckSum = 'Y', @LogToTable = 'Y'" -b) ELSE IF Mon == %DATE:~0,3% (sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d ohsu_sqltab -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = 'SYSTEM_DATABASES', @Directory = N'\\msbdd1b\msb_sql\Prod\Weekly', @BackupType = 'FULL', @Verify = 'Y', @CopyOnly = 'N', @Compress = 'N', @CleanupTime = 2184, @CheckSum = 'Y', @LogToTable = 'Y'" -b) ELSE (sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d ohsu_sqltab -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = 'SYSTEM_DATABASES', @Directory = N'\\msbdd1b\msb_sql\Prod\Daily', @BackupType = 'FULL', @Verify = 'Y', @CopyOnly = 'N', @Compress = 'N', @CleanupTime = 504, @CheckSum = 'Y', @LogToTable = 'Y'" -b)
					--IF 01 == %date:~7,2% (sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d ohsu_sqltab -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''SYSTEM_DATABASES'', @Directory = N''\\msbdd1b\msb_sql\Prod\Monthly'', @BackupType = ''FULL'', @Verify = ''N'', @CopyOnly = ''N'', @Compress = ''N'', @CleanupTime = 9672, @CheckSum = ''Y'', @LogToTable = ''Y''" -b) ELSE IF Mon == %DATE:~0,3% (sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d ohsu_sqltab -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''SYSTEM_DATABASES'', @Directory = N''\\msbdd1b\msb_sql\Prod\Weekly'', @BackupType = ''FULL'', @Verify = ''Y'', @CopyOnly = ''N'', @Compress = ''N'', @CleanupTime = 2184, @CheckSum = ''Y'', @LogToTable = ''Y''" -b) ELSE (sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d ohsu_sqltab -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''SYSTEM_DATABASES'', @Directory = N''\\msbdd1b\msb_sql\Prod\Daily'', @BackupType = ''FULL'', @Verify = ''Y'', @CopyOnly = ''N'', @Compress = ''N'', @CleanupTime = 504, @CheckSum = ''Y'', @LogToTable = ''Y''" -b)

		Select @JobID = job_id FROM msdb.dbo.sysjobs WHERE [name] = 'DatabaseBackup - USER_DATABASES - DIFF'
			EXEC msdb.dbo.sp_update_jobstep @job_id=@JobID, @step_id=1 , 
					@command=N'sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d ohsu_sqltab -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''USER_DATABASES'', @Directory = N''\\msbdd1b\msb_sql\Prod\Daily'', @BackupType = ''DIFF'', @Verify = ''Y'', @Compress = ''N'', @CleanupTime = 504, @CheckSum = ''Y'', @LogToTable = ''Y''" -b'
					,@retry_attempts=2
					,@retry_interval=2
					--sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d ohsu_sqltab -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = 'USER_DATABASES', @Directory = N'\\msbdd1b\msb_sql\Prod\Daily', @BackupType = 'DIFF', @Verify = 'Y', @Compress = 'N', @CleanupTime = 504, @CheckSum = 'Y', @LogToTable = 'Y'" -b

		Select @JobID = job_id FROM msdb.dbo.sysjobs WHERE [name] = 'DatabaseBackup - USER_DATABASES - FULL'
			EXEC msdb.dbo.sp_update_jobstep @job_id=@JobID, @step_id=1 , 
					@command=N'IF 01 == %DATE:~7,2% (sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d ohsu_sqltab -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''USER_DATABASES'', @Directory = N''\\msbdd1b\msb_sql\Prod\Monthly'', @BackupType = ''FULL'', @Verify = ''N'', @CopyOnly = ''N'', @Compress = ''N'', @CleanupTime = 9672, @CheckSum = ''Y'', @LogToTable = ''Y''" -b) ELSE IF Mon == %DATE:~0,3% (sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d ohsu_sqltab -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''USER_DATABASES'', @Directory = N''\\msbdd1b\msb_sql\Prod\Weekly'', @BackupType = ''FULL'', @Verify = ''Y'', @CopyOnly = ''N'', @Compress = ''N'', @CleanupTime = 2184, @CheckSum = ''Y'', @LogToTable = ''Y''" -b) ELSE (sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d ohsu_sqltab -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''USER_DATABASES'', @Directory = N''\\msbdd1b\msb_sql\Prod\Daily'', @BackupType = ''FULL'', @Verify = ''Y'', @CopyOnly = ''N'', @Compress = ''N'', @CleanupTime = 504, @CheckSum = ''Y'', @LogToTable = ''Y''" -b)'
					,@retry_attempts=2
					,@retry_interval=2
					--IF 01 == %DATE:~7,2% (sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d ohsu_sqltab -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = 'USER_DATABASES', @Directory = N'\\msbdd1b\msb_sql\Prod\Monthly', @BackupType = 'FULL', @Verify = 'N', @CopyOnly = 'Y', @Compress = 'N', @CleanupTime = 9672, @CheckSum = 'Y', @LogToTable = 'Y'" -b) ELSE IF Mon == %DATE:~0,3% (sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d ohsu_sqltab -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = 'USER_DATABASES', @Directory = N'\\msbdd1b\msb_sql\Prod\Weekly', @BackupType = 'FULL', @Verify = 'Y', @CopyOnly = 'N', @Compress = 'N', @CleanupTime = 2184, @CheckSum = 'Y', @LogToTable = 'Y'" -b) ELSE (sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d ohsu_sqltab -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = 'USER_DATABASES', @Directory = N'\\msbdd1b\msb_sql\Prod\Daily', @BackupType = 'FULL', @Verify = 'Y', @CopyOnly = 'N', @Compress = 'N', @CleanupTime = 504, @CheckSum = 'Y', @LogToTable = 'Y'" -b)
...
PRINT 'PRD - Ola Job Steps Created'